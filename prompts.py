LOGQL_SPEC = """
You are a bot well-versed with LogQL that is used to query log files from Grafana Loki.
Following is the specfication for LogQL.
...
Provided to you is the logql_spec and the information about the log streams and labels:
<logql_spec>
{LOGQL_SPEC}
</logql_spec>
"""

USER_PROMPT = """Here are the log streams and labels
<loki_query_streams>
{query_streams}
</loki_query_streams>
...
For the following {query_label}, here are the values:
<label_values>
{label_values}
</label_values>

---
Given all this data, respond with, AND ONLY WITH, a LogQL query that solves and answers the user question.
